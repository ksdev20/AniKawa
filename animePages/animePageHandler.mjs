...
import { createAnimeCard, getScore, genreCheck, toggleWatchlist, slidersHandler, fetchWatchlist, checkWatchlist } from "../anime-card.mjs";
...
function loadMainData(newFirst = false) {
    ...
    if (wBtnAnimepage) {
        wBtnAnimepage.addEventListener('click', () => {
            toggleWatchlist(animeJson, 'watchlist', wBtnAnimepage).then(() => fetchWatchlist(true));
        });
    }
